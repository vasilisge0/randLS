...
void configure_magma(magma_info& magma_config)
{
    ...
    magma_queue_create_from_cuda(magma_config.cuda_device, magma_config.cuda_stream, magma_config.cublas_handle,
                                magma_config.cusparse_handle, &magma_config.queue);
    curandCreateGenerator(&magma_config.rand_generator, CURAND_RNG_PSEUDO_DEFAULT);
    // 1234ULL default seed for testing, time(NULL)
    // curandSetPseudoRandomGeneratorSeed(magma_config.rand_generator, 1234ULL); // original
