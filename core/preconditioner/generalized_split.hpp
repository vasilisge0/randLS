...
class generalized_split : public preconditioner<value_type_in, value_type, index_type> {
public:
    ...
    generalized_split(std::shared_ptr<Context> context) { 
        this->set_type();
        this->precond_type_ = generalized_split;
        this->sampling_coeff_ = 1.5;
    ...
    generalized_split(matrix::dense<value_type>& sketch_mtx) {
        sketch_mtx_ = std::move(sketch_mtx);
        this->set_type();
    }

    generalized_split(std::shared_ptr<matrix::dense<value_type>> mtx,
        ...
        this->precond_mtx_ = std::shared_ptr<matrix::dense<value_type>>(new matrix::dense<value_type>());
    ...
    void generate(matrix::dense<value_type>* mtx) {
        ...
        this->precond_mtx_ = std::shared_ptr<matrix::dense<value_type>>(new matrix::dense<value_type>({sampled_rows, size[1]}));
    ...
    generalized_split<value_type_in, value_type, index_type>* get() { return this; }

    matrix::dense<value_type>* get_mtx() { return this->precond_mtx_.get(); }

    static std::unique_ptr<generalized_split<value_type_in, value_type, index_type>> create(std::shared_ptr<Context> context) {
        return std::unique_ptr<generalized_split<value_type_in, value_type, index_type>>(new generalized_split<value_type_in, value_type, index_type>(context));
    }
