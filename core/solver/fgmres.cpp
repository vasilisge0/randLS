...
void givens_qr(dim2 size, value_type* hessenberg_mtx, index_type ld_h,
...
template <typename value_type_in, typename value_type, typename index_type,
          ContextType device_type>
void initialize(std::shared_ptr<Context<device_type>> context, dim2 size,
                value_type* mtx, value_type* sol, value_type* rhs,
                preconditioner::preconditioner<value_type, index_type>* precond,
                ...
                magma_queue_t& queue)
{
    ...
    // rls::utils::convert(context, global_length, 1, vectors->residual,
    // global_length, residual_in, global_length); precond->apply(MagmaTrans,
    // residual_in, vectors->inc); rls::utils::convert(context, global_length,
    // 1, residual_in, global_length, vectors->residual, global_length);
    // }
    // else {
    // }
    ...
    index_type cur_iter, magma_queue_t queue)
{
    ...
    if (typeid(value_type_in) != typeid(value_type)) {
        ...
        rls::utils::convert(context, global_len, 1, z, global_len, z_in,
                            global_len);
        rls::utils::convert(context, global_len, 1, w, global_len, w_in,
                            global_len);
        magma_queue_sync(queue);
        // fix precisions issues
        fgmres::gemv(MagmaNoTrans, size[0], size[1], one, mtx_in, size[0], z_in,
                     1, zero, w_in, 1, temp_in, queue);
        magma_queue_sync(queue);
        rls::utils::convert(context, global_len, 1, w_in, global_len, w,
                            global_len);
        magma_queue_sync(queue);
    } else {
        value_type one = 1.0;
    ...
    index_type cur_iter, index_type max_iter, magma_queue_t queue)
{
    ...
    for (index_type i = 0; i < cur_iter + 1; i++) {
        ...
        blas::axpy(global_len, -hessenberg_mtx[i + ld * cur_iter], v, 1, w, 1,
                   queue);
    }
    ...
    fgmres::givens_qr(size, hessenberg_mtx, (max_iter + 1),
                      vectors->hessenberg_rhs, cur_iter, max_iter,
                      *vectors->givens_cache.get());
    magma_queue_sync(queue);
    ...
    blas::trsv(MagmaUpper, MagmaNoTrans, MagmaNonUnit, cur_iter + 1,
    ...
    blas::gemv(MagmaNoTrans, global_len, cur_iter + 1, 1.0, vectors->z_basis,
               global_len, hessenberg_rhs_gpu, 1, 0.0, sol->get_values(), 1,
               queue);
}
...
void run_fgmres(matrix::Dense<value_type, device_type>* mtx,
                matrix::Dense<value_type, device_type>* rhs,
                matrix::Dense<value_type, device_type>* sol,
                preconditioner::preconditioner<value_type, index_type,
                                               device_type>* precond,
                fgmres::temp_scalars<value_type, index_type>* scalars,
                fgmres::temp_vectors<value_type_in, value_type, index_type,
                                     device_type>* vectors,
                ...
                double* resnorm, magma_queue_t queue, double* t_solve)
{
    ...
    *t_solve = 0.0;
    ...
    auto global_length = size[0] + size[1];
    fgmres::initialize(context, size, mtx->get_values(), sol->get_values(),
                       rhs->get_values(), precond, scalars, vectors, queue);
    double t = magma_sync_wtime(queue);
    while (1) {
        fgmres::step_1(context, size, mtx, precond, scalars, vectors, *iter,
                       queue);
        fgmres::step_2(context, size, mtx, sol, precond, scalars, vectors,
                       *iter, max_iter, queue);
        ...
        blas::scale(global_length, 1.0 / scalars->h,
                    &vectors->v_basis[global_length * (*iter)], vectors->inc,
                    queue);
    }
...
template void run_fgmres(
    ...
    preconditioner::preconditioner<double, magma_int_t, CUDA>* precond,
...
template void run_fgmres(
    ...
    preconditioner::preconditioner<float, magma_int_t, CUDA>* precond,
