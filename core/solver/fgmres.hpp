...
struct workspace {
    ...
    workspace(std::shared_ptr<Context<device_type>> context, dim2 size,
              int max_iter, magma_queue_t& queue)
    {
        ...
        tmp_cpu = matrix::Dense<value_type, CPU>::create(context_cpu,
                                                         {max_iter_ + 1, 1});
    }
...
struct temp_vectors {
    ...
    temp_vectors(std::shared_ptr<Context<device_type>> context, dim2 size,
                 int max_iter, magma_queue_t& queue)
    {
        std::shared_ptr<Context<CPU>> context_cpu = Context<CPU>::create();
        context_ = context;
...
public:
    ...
    static std::unique_ptr<
        Fgmres<value_type_in, value_type, index_type, device_type>>
           ...
           double tolerance_in)
    {
        return std::unique_ptr<
            Fgmres<value_type_in, value_type, index_type, device_type>>(
            new Fgmres<value_type_in, value_type, index_type, device_type>(
                precond_in, mtx, rhs, tolerance_in));
    }
        ...
        if (use_precond_) {
            ...
            for (auto i = 0; i < this->logger_.warmup_runs_; i++) {
                ...
                run_fgmres(mtx_.get(), glb_rhs_.get(), glb_sol_.get(),
                           static_cast<preconditioner::preconditioner<
                               value_type, index_type, device_type>*>(precond_),
                           &scalars_, vectors_.get(), this->get_max_iter(),
                           this->get_tolerance(), &iter, &resnorm,
                           this->context_->get_queue(),
                           &this->logger_.runtime_);
            }
            ...
            this->logger_.runtime_ =
                this->logger_.runtime_ / this->logger_.runs_;
    ...
    void generate()
    {
        ...
        sol_ = matrix::Dense<value_type, device_type>::create(this->context_,
                                                              {num_cols, 1});
        sol_->zeros();
        glb_sol_ = matrix::Dense<value_type, device_type>::create(
            this->context_, {global_len, 1});
        glb_sol_->zeros();
        glb_rhs_ = matrix::Dense<value_type, device_type>::create(
            this->context_, {global_len, 1});
        glb_rhs_->zeros();

        auto norm_rhs = blas::norm2(num_rows, rhs_->get_values(), 1, queue);
        blas::copy(num_rows, rhs_->get_values(), 1, glb_rhs_->get_values(), 1,
                   queue);
        blas::copy(num_cols, sol_->get_values(), 1,
                   &glb_rhs_->get_values()[num_rows], 1, queue);
        auto norm_rhs2 =
            blas::norm2(global_len, glb_rhs_->get_values(), 1, queue);
        this->max_iter_ = num_cols;

        vectors_ =
            std::shared_ptr<fgmres::temp_vectors<value_type_in, value_type,
                                                 index_type, device_type>>(
        ...
        utils::convert(this->context_, mtx_->get_size()[0], mtx_->get_size()[1],
                       this->mtx_->get_values(), mtx_->get_ld(),
                       vectors_->mtx_in, this->mtx_->get_ld());
