...
//    //std::string filename_sketch = "sparse_sketch.mtx";
//    //auto sketch = rls::share(sketch_type::create(context, k, filename_sketch));
//
...
//    //solver_config->set_tolerance(1e-2);
//    solver_config->set_iterations(100);         // @CHECK (!!) need to check with matrix size bounds.
//    //solver_config->set_iterations(2);         // @CHECK (!!) need to check with matrix size bounds.
...
//    solver->run();
    ...
    //gko::dim<2> s1 = {1, 2};
    //auto rhs_tmp = gko::share(gko::matrix::Dense<double>::create(exec, s1));
